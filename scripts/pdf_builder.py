...
def _render_book(
    ...
    if comfyui_client:
        try:
            comfyui_client.validate_connection()
        except Exception as exc:
            print(f"⚠️ ComfyUI connection check failed: {exc}")
            print(
                "⚠️ Checklist: run ComfyUI with --listen, confirm URL/port, verify workflow JSON path, "
                "and ensure checkpoint file exists in ComfyUI models/checkpoints."
            )
            comfyui_client = None
        ...
        if comfyui_client:
            try:
                ...
                if not comfyui_warning_shown:
                    ...
                    print(
                        "⚠️ Fix: check SaveImage node in workflow, checkpoint name, and that your workflow "
                        "uses '{{prompt}}' in the text node."
                    )
